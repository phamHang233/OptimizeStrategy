...
from src.constants.network_constant import URL_PROTOCOL
import requests
...
def get_high_low_day_price_datas(pool, protocol, from_date, to_date):
    url = URL_PROTOCOL.mapping.get(protocol)
    query = """
    query PoolDayDatas($id: ID!, $fromdate: Int!, $todate: Int!) {

        poolDayDatas(where: {pool: $id, date_gte: $fromdate, date_lte: $todate})
        {
        high
        low
          pool {
